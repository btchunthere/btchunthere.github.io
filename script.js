...
const updateBalance = (type, balance) => {
    document.getElementById(`${type}Balance`).innerText = balance > 0 ? `${balance} BTC` : '0 BTC';
};
...
const generateKey = () => {
    ...
    const { address: compressedAddress } = bitcoinjs.payments.p2pkh({ pubkey: keyPair.publicKey });
    const uncompressedKeyPair = bitcoinjs.ECPair.fromPrivateKey(keyPair.privateKey, { compressed: false });
    const { address: uncompressedAddress } = bitcoinjs.payments.p2pkh({ pubkey: uncompressedKeyPair.publicKey });
    const { address: bech32Address } = bitcoinjs.payments.p2wpkh({ pubkey: keyPair.publicKey });
